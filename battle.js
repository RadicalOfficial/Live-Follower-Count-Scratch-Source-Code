...
function fetchAndSetCount(icons, classe) {
  return new Promise(async (resolve, reject) => {
    try {
      const firstResponse = await fetch(`https://corsproxy.io/?https://scratchstats.com/api2/live/getid/${icons}`);
      const firstData = await firstResponse.json();

      const id = firstData.id;

      const secondResponse = await fetch(`https://corsproxy.io/?https://scratchstats.com/api2/live/id/${id}`);
...
function compareUsers() {
  ...
  if (su1 == "Enter a username..." || su2 == "Enter a username...") {
    if (!document.getElementById("firstu").value == "" || !document.getElementById("secondu").value == "") {
      alert("You must click enter on both usernames in order to confirm.");
      return;
    }
    alert("Your usernames are empty. Please enter them in order to start the battle.");
    return;
  }
  console.log("Starting the battle. Please wait...");
  document.getElementById("header").innerHTML = "Please wait. <br>We are setting up the comparison...";
